...
public class DefaultHelpFormatter : IHelpFormatter
{
    ...
    public IEnumerable<IEmbed> GetCommandHelp(IEnumerable<IChildNode> subCommands)
    {
        ...
        if (!subCommandArray.OfType<IParentNode>().Any())
        {
            ...
            for (var i = 0; i < sca.Length; i++)
                yield return (GetCommandHelp(sca[i]) as Embed)! with
                {
                    Title = $"Help for {sca[0].Key} (overload {i + 1} of {sca.Length})"
                };
    ...
    public IEnumerable<IEmbed> GetTopLevelHelpEmbeds(IEnumerable<IGrouping<string, IChildNode>> commands)
    {
        ...
        else
        {
            ...
            foreach (var group in commandArray)
            {
                ...
                foreach (var command in group)
                {
                    ...
                    var categoryName = _options.CommandCategories.FirstOrDefault(c =>
                        c.Equals(categoryAttribute.Category, StringComparison.OrdinalIgnoreCase));
                ...
                if (!categorized.ContainsKey(category))
                    categorized.Add(category, new List<IGrouping<string, IChildNode>>());

                categorized[category].Add(group);
            }

            var orderedCategories = categorized.Skip(1).OrderBy(c => _options.CommandCategories.IndexOf(c.Key))
                .Append(categorized.First());
        ...
        var embed = GetBaseEmbed() with
        {
            ...
            Footer = new EmbedFooter(
                "Specify a command for more information. Commands with \"*\" are groups that can be used like commands.")
        };

        yield return embed;
    }

    private Embed GetBaseEmbed()
    {
        return new Embed { Colour = Color.DodgerBlue };
    }
    ...
    private void AddCommandUsage(StringBuilder builder, IChildNode command)
    {
        ...
        foreach (var parameter in cn.Shape.Parameters)
        {
            ...
            builder.AppendLine(
                $" {(string.IsNullOrEmpty(parameter.Description) ? "No description" : parameter.Description)}");
            builder.AppendLine();
        }
    }
    ...
    private void AddRequiredPermissions(StringBuilder builder, IChildNode node)
    {
        ...
        if (node is GroupNode gn)
        {
            ...
            rpa = (gn.Children.First(child => child is CommandNode) as CommandNode)!
                .FindCustomAttributeOnLocalTree<RequireDiscordPermissionAttribute>();
        }
