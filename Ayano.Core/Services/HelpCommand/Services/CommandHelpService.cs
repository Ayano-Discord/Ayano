...
#pragma warning restore CS8509
    ...
    public async Task<(IEnumerable<IChildNode> Nodes, ConditionAttribute? UnsatisfiedCondition)>
        EvaluateNodeConditionsAsync(IReadOnlyList<IChildNode> nodes)
    {
            ...
            foreach (var setCondition in conditions)
            {
                ...
                if (!conditionServices.Any())
                    throw new InvalidOperationException(
                        $"Command was marked with \"{conditionType.Name}\", but no service was registered to handle it.");

                foreach (var condition in conditionServices)
                {
                    var result = await (ValueTask<Result>)conditionMethod!.Invoke(condition,
                        new object[] { setCondition, CancellationToken.None })!;
