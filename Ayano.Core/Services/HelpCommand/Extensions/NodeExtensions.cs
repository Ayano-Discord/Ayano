...
public static class GroupNodeExtensions
{
    ...
    public static TAttribute? GetAttributeFromTree<TAttribute>(this IChildNode startingNode)
        where TAttribute : Attribute
    {
        ...
        {
            if ((cn.CommandMethod.GetCustomAttribute<TAttribute>() ?? cn.GroupType.GetCustomAttribute<TAttribute>()) is
                { } commandAttribute)
                return commandAttribute;
        }
        else if (startingNode is GroupNode gn)
        {
            if (gn.GroupTypes.Select(gt => gt.GetCustomAttribute<TAttribute>()).FirstOrDefault(gt => gt is not null) is
                { } groupAttribute)
    ...
    public static IEnumerable<TAttribute> GetAttribtuesFromTree<TAttribute>(this IChildNode startingNode,
        bool returnAnscestors) where TAttribute : Attribute
    {
        IChildNode? parent = null;
        ...
        switch (startingNode)
        {
            case CommandNode cn:
            {
                if ((cn.CommandMethod.GetCustomAttribute<TAttribute>() ?? cn.GroupType.GetCustomAttribute<TAttribute>())
                    is { } commandAttribute)
                {
            ...
            case GroupNode gn:
            {
                var attributes = gn.GroupTypes.Select(gt => gt.GetCustomAttribute<TAttribute>())
